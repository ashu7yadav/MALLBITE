...
// Request logger for easy dev visibility
...
app.get("/api/health", (req, res) => {
  res.json({ status: "OK", platform: "MALLBITE API v1.0", timestamp: new Date().toISOString() });
});
...
app.post("/api/demo/simulate-step", (req, res) => {
  ...
  if (step === 1) {
    ...
    // Delivery runner is on the way to table
    const dt = dataStore.getDeliveryTasks().find(t => t.masterOrderId === latestOrder.id);
    if (dt) {
...
app.listen(PORT, () => {
  console.log(`🚀 MALLBITE Backend Server running on http://localhost:${PORT}`);
});
